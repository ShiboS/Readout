...
def Save_dB_noT():
    ...
    for i in range(0, int(VNA.GetNumPoint())):
        fwrite.writerow([x_array[i], y_array_log[i]])
"""
frequency = 5.1722363E9
Measure_KID(frequency, -55)
Save_KID()
Measure_KID(frequency, -50)
Save_KID()
Measure_KID(frequency, -45)
Save_KID()
Measure_KID(frequency, -40)
Save_KID()
Measure_KID(frequency, -35)
Save_KID()
Measure_KID(frequency, -30)
Save_KID()
Measure_KID(frequency, -25)
Save_KID()
Measure_KID(frequency, -20)
Save_KID()
Measure_KID(frequency, -15)
Save_KID()
Measure_KID(frequency, -10)
Save_KID()
Measure_KID(frequency, -5)
Save_KID()
"""
#Save_dB_noT()
# Temperature control
...
for Temp in range(StartTemp, EndTemp+Step, Step):
    ...
    for i in range(1,1800):
        ...
        if ((float(LS370.GetTemp("4"))<=Temp/1000.*1.005) and (float(LS370.GetTemp("4"))>=Temp/1000.*0.995)):
            time.sleep(10)
            if ((float(LS370.GetTemp("4"))<=Temp/1000.*1.005) and (float(LS370.GetTemp("4"))>=Temp/1000.*0.995)):
                time.sleep(10)
                if ((float(LS370.GetTemp("4"))<=Temp/1000.*1.005)and (float(LS370.GetTemp("4"))>=Temp/1000.*0.995)):
