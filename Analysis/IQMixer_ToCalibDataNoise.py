...
filename = '/EllipseFit_3dBm_3000MHz_8000MHz'
RF_Power = '3dBm'
...
data_file = 'Noise_19dBm_3868MHz_50K_1S'
data = []
with open(data_file + '.csv','r') as f:
    for line in f:
        data.append(map(str,line.split(',')))
        
freq = [float(data[i][0]) for i in range(22,len(data))]
I = [float(data[i][1]) for i in range(22,len(data))]
