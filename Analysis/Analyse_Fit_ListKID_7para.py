...
### Read filename list
data_name = []
folder = "20160516_Nb154nmCry3_7/"
readpower = '-40dBm'

name_of_list = '20160518_' + readpower + '_list.csv'
...
for filename in data_name:
    ...
    Fit_result.append(Analyse_Fit_SingleKID.Fit_7parameter(folder, filename))
...
def save_fit(data, folder, power):
    ...
    fwrite.writerow( ('Power(dBm)', 'Temperature(mK)', 'fr from mag(S21)', 'fr from phase fitting', 'standard 1 sigma err of fr', 'Qr from phase fitting', 'standard 1 sigma err of Qr', 'Qc from phase fitting', 'Qi from phase fitting', 'name of data file', "deltaf/fo from mag(S21)", "deltaf/fo from phase fitting"))
    
    # Write number-of-points rows
    for i in range(0, len(data)):
        fwrite.writerow([data[i][0], data[i][1], data[i][2], data[i][3], data[i][4], data[i][5], data[i][6], data[i][7], data[i][8], data[i][9], (data[i][2]-data[0][2])/data[0][2], (data[i][3]-data[0][3])/data[0][3]])
